...
class meridianModel extends Database
{
    ...
    {
        $query = "SELECT i.id, items.img, i.id_item, i.status, items.name FROM `inventories` as i left join items on i.id_item = items.id WHERE id_user = '" . $id_user . "' and items.type_equipment like 'mach' and i.status = 1";
    ...
    public function updateStatusMeridianToWear($id_user, $id_inventory, $id_item)
    {
        ...
        if ($count <= 0) {
            $query = "UPDATE `inventories` SET `status`=0 WHERE id_user = '" . $id_user . "' and id = '" . $id_inventory . "'";
